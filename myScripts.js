...
$('.toTop, .ToTop-fixed').click(function () {
    $('html,body').scrollTop(0);
});
$('.toTop, .ToTop-fixed').contextmenu(function (e) {
    e.preventDefault();
...
document.addEventListener('scroll', function () {
    ...
    $('.ToTop-fixed').css({ backgroundImage: 'conic-gradient(var(--primaryClr), var(--primaryClr2), var(--primaryClr) ' + currentScroll + '%, transparent ' + currentScroll + '%,  transparent 100%)' });
    // Toggle to top
    if (scrollY < (($(window).height() * 3) + 100)) {
        $('.ToTop-fixed').css({ visibility: "hidden" });
    }
    else {
        $('.ToTop-fixed').css({ visibility: "visible" });
    }
});
...
const todayMonth = (today.getMonth() + 1).toString().padStart(2, '0'); // Months are zero-based
const todayDay = today.getDate().toString().padStart(2, '0');
const maxDate = `${todayYear}-${todayMonth}-${todayDay}`;
// Set the maximum date for the input field
document.querySelectorAll('.max-date-today').forEach(el => {
...
inputsWithMaxlength.forEach(input => {
    input.addEventListener('input', function () {
        ...
        if (this.value.length > maxLength) {
            this.value = this.value.slice(0, maxLength); // Trim the value
        }
    });
});
...
// Dynamically add a loader to the page
function addLoader() {
    // Create loader elements
    var loaderContainer = $('<div class="flex-center Loading_fix"></div>');
    var loadingMotionWave = $('<div class="loading-motion-boxes"></div>');

    // Add bars to loadingMotionWave
    for (var i = 0; i < 5; i++) {
        loadingMotionWave.append('<div class="box"></div>');
    }
...
function toggle_prev() {
    var toggler_Elem = $(this),
        toggling_Elem = toggler_Elem.prev();
    toggling_Elem.collapse('toggle');
}
$('.toggle-prev').click(toggle_prev);
...
$('.side-tools_toggler').click(function () {
    $(this).next().toggleClass('working');
});
...
const show_toast = (duration, message) => {
...
window.onscroll = function () { // or "onscroll = function() {"
    ...
    let currentPosit = scrollY,
        webNavigators = $("nav.navbar, #stickyNavbar");
...
/**
* Circular gallery navigator
*/

$('.circular-gallery-navigator > .switcher').click(function (e) {
    const dis = $(this).parent(),
        theGallery = dis.attr('data-control'),
        allElems = $(theGallery).find('> .circular-gallery-item'),
        allElemsLen = allElems.length;
    allElems.each(function () {
        const theImage = $(this);
        let nthChild = parseInt(theImage.css('--_child'));
        if ($(e.target).hasClass('switcher-prev')) {
            nthChild += 1;
        } else if ($(e.target).hasClass('switcher-next')) {
            nthChild -= 1;
        }
        theImage.css('--_child', nthChild);
    });
});
...
function month_nm_to_num(xnum) {
    xnum = xnum.replace('January', '01');
    xnum = xnum.replace('February', '02');
    xnum = xnum.replace('March', '03');
    xnum = xnum.replace('April', '04');
    xnum = xnum.replace('May', '05');
    xnum = xnum.replace('June', '06');
    xnum = xnum.replace('July', '07');
    xnum = xnum.replace('August', '08');
    xnum = xnum.replace('September', '09');
    xnum = xnum.replace('October', '10');
    xnum = xnum.replace('November', '11');
    xnum = xnum.replace('December', '12');
    return xnum;
}
...
function preview_file(previewSpace) {
    ...
    thePreview.html('');
    thePreview.attr('src', '');
    let selectedSong = $('#my-compositions .active'),
        songLinkID = get_gDrive_file_id(selectedSong.find('.composition-file').text().trim()),
...
document.onscroll = function () {
    ...
    // Float playing audio
    var audioSection = $('.audio-songs__media-player');
